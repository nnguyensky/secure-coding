...
[![Tests](https://img.shields.io/badge/Tests-489%20Passing%20(100%25)-brightgreen?style=for-the-badge&logo=checkmarx&logoColor=white)](hooks/test.js)
...
13 language templates · 376 patterns mapped to CWE and OWASP Top 10:2025 · 489
...
```bash
node hooks/test.js     # expect: pass=489 fail=0
...
| **🧹 Quality** | `node hooks/clean.js <paths...>` | Lints against 14 clean code standards; exits 2 on violations. |
| | `node hooks/clean.js --all` | Lints every tracked file (`npm run clean`). Blocks only at the `failOn` severity. |
...
| | `node hooks/test.js` | Executes the full 489-test self-check suite. |
...
`secure-coding` includes a native, zero-dependency JSON-RPC 2.0 MCP server ([`mcp/server.js`](mcp/server.js)). It gives AI agents direct access to 9 security tools without running subshells or polluting chat context.
...
Verify all 489 test assertions, pattern compilation, secret masking, and MCP tools:
